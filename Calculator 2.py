...
fen.configure(bg="#212420",padx=5,pady=5)       #padx and pady create an empty space near the edge of the window



#We then add rows and columns to it, for our buttons
...
#We create dictionaries that we'll use for our button (so we don't have to write the same thing 100x in a row)
...
#We'll need a variable for the screen of our calculator.

var=tk.StringVar()      #However, tkinter only allows their own special variable : tk.StringVar
var.set(0)              #Intially we want the screen to display "0", so we set its value to "0"


#We then need a screen to display the results of our calculator. we define it as a label (a text area)
...
num2=0                          #It's more convenient to work with string, but using float as their base value will allow us to detect if it has been modifie (and is a string) or not (and is a float)
